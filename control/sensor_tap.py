...
class AttitudeSensorTap:
    """Non-blocking subscriber that keeps the latest attitude (roll/pitch/yaw) sample."""
    ...
    def __init__(self, endpoint: str, *, conflate: bool = True, rcv_hwm: int = 10):
        ...
        self.last_roll_deg: Optional[float] = None
    ...
    def poll(self, *, max_msgs: int = 50) -> None:
        ...
        while n < max_msgs:
            ...
            rpy = (msg or {}).get("rpy_deg")
            if not isinstance(rpy, dict):
                continue

            try:
                self.last_roll_deg = float(rpy.get("roll", 0.0))
                self.last_pitch_deg = float(rpy.get("pitch", 0.0))
                self.last_yaw_deg = float(rpy.get("yaw", 0.0))
                self.last_ts = time.time()
            except Exception:
                continue

    def age_s(self, now: Optional[float] = None) -> Optional[float]:
        """Age since last valid attitude sample (seconds)."""
        if self.last_ts is None:
            return None
        if now is None:
            now = time.time()
