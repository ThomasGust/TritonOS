...
class ICM20602:
    ...
    def read_all(self):
        """Read accel + gyro from a single I2C burst (same sample instant).

        Returns (Vec3_accel, Vec3_gyro).  Avoids the temporal skew that
        happens when read_accel() and read_gyro() each do their own 14-byte
        read from the same register block.
        """
        b = self._read14()
        ax = _twos((b[0] << 8) | b[1])
        ay = _twos((b[2] << 8) | b[3])
