    ...
    if bool(getattr(cfg, "ATTITUDE_ESTIMATOR_ENABLE", True)):
        try:
            ...
            attitude_cfg = RollPitchConfig(
                ...
                vehicle_roll_axis=str(getattr(cfg, "ATTITUDE_VEHICLE_ROLL_AXIS", "x")),
                roll_sign=float(getattr(cfg, "ATTITUDE_ROLL_SIGN", 1.0)),
                pitch_sign=float(getattr(cfg, "ATTITUDE_PITCH_SIGN", 1.0)),
                ...
                yaw_mag_smooth_tau_s=float(getattr(cfg, "ATTITUDE_YAW_MAG_SMOOTH_TAU_S", 0.65)),
                yaw_reference_samples=int(getattr(cfg, "ATTITUDE_YAW_REFERENCE_SAMPLES", 30)),
